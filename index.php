...
$stmt=$pdo->prepare('SELECT COUNT(*) FROM cliente WHERE data=? AND hora=?');
...
if($count>0){
    ...
    $stmt=$pdo->prepare('INSERT INTO cliente(nome, email, telefone, data, hora) VALUES (:nome, :email, :telefone, :data, :hora)');
    $stmt->execute(['nome'=>$nome, 'email'=>$email, 'telefone'=>$telefone, 'data'=>$data, 'hora'=>$hora]);

    if($stmt->rowCount()){
        echo'<span>Compromisso agendado co sucesso!</span>';
    }else{
